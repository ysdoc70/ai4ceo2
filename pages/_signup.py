...
st.caption(
    "이미 가입한 ID로 같은 ID·새 비밀번호를 입력해 **비밀번호 재설정** 버튼을 누르면 비밀번호만 갱신됩니다. "
    "기존 세션·문서 데이터는 그대로 유지됩니다."
)
        ...
        if _validate(u):
            try:
                ...
                client.table("app_users").insert(
                    {"username": u, "password_hash": ph, "salt": sh}
                ).execute()
                ...
                if "duplicate" in msg.lower() or "23505" in msg:
                    st.error(
                        "이미 사용 중인 사용자 ID입니다. "
                        "이 ID 의 비밀번호를 잊었다면 우측의 **비밀번호 재설정** 버튼을 사용하세요."
                    )
                else:
        ...
        if _validate(u):
            try:
                ...
                if not (exist.data or []):
                    st.error("해당 ID 로 가입된 계정이 없습니다. 먼저 **가입 요청** 을 진행하세요.")
                else:
                    ph, sh = hash_password_new(pw)
                    client.table("app_users").update(
                        {"password_hash": ph, "salt": sh}
                    ).eq("username", u).execute()
                    st.success("비밀번호를 재설정했습니다. 메인 화면에서 새 비밀번호로 로그인하세요.")
            except APIError as e:
                st.error(f"재설정 오류: {getattr(e, 'message', str(e))}")
            except Exception as e:
                st.error(f"재설정 오류: {e}")
